...
def tga_dissimilarity(
    np.ndarray[np.double_t, ndim=2] seq1,
    np.ndarray[np.double_t, ndim=2] seq2,
    double sigma,
    int triangular):
    ...
    Parameters
    ----------
    seq1: 2D, C-contiguous (ie. read line by line) numpy array of doubles,
          T1 x d multivariate (dimension d) time series of duration T1

    seq2: 2D, C-contiguous (ie. read line by line) numpy array of doubles,
          T2 x d multivariate (dimension d) time series of duration T2

    sigma: double,
           bandwitdh of the inner distance kernel, generally set in
           {0.1, 0.5, 1, 2, 5, 10} * median(dist(x, y)) * sqrt(median(length(x)))

    triangular: int,
                parameter to restrict the paths (closer to the diagonal) used
                by the kernel, good practice: {0.25, 0.5} * median(length(x))
    ...
    Notes
    -----
    Behavior w.r.t. values of triangular:
        * 0: use all paths
    ...
    assert seq1.flags['C_CONTIGUOUS'] and seq2.flags['C_CONTIGUOUS'], \
            "Invalid series: not C-contiguous"
